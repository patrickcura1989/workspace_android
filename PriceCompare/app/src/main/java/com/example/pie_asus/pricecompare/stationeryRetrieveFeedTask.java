...
class stationeryRetrieveFeedTask extends AsyncTask<Void, Void, String>
{
    ...
    public stationeryRetrieveFeedTask(Preference preference, String searchInput)
    {
    ...
    {
        stationeryRetrieveFeedTask example = new stationeryRetrieveFeedTask();
        String response = null;
        try
        {
            response = example.run("http://www.warehousestationery.co.nz/search?q=" + this.searchInput + "#q=" + this.searchInput + "&srule=Lowest-Highest&sz=9000&start=0");
        }
        catch (IOException e)
        {
            e.printStackTrace();
                ...
                SAXParserImpl.newInstance(null).parse(stream,
                        ...
                        new DefaultHandler()
                        {
                            ...
                            public void startElement(String uri, String localName, String name, Attributes a)
                            {
                                ...
                                {
                                    if ("name-link gtm-product-click".equals(a.getValue("class")))
                                    {
                                ...
                                {
                                    if ("price-sales".equals(a.getValue("class")))
                                    {
                                        isPrice = true;
                                    }
                                }
